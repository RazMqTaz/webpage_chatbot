...
@click.command()
@click.option("--target-url", required=True, help="The root URL of the website used for document context (e.g. https://soniox.com)")
@click.option("--top-k", default=5, show_default=True, help="Number of top matching document chunks to retrieve from ChromaDB")

def main(target_url: str, top_k: int):
    name = get_domain_name(target_url)
